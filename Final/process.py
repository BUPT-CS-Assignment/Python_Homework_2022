...
def extract():
    df_new = pd.DataFrame(columns=['id','name','city','cbd','brand','area','orien','room','price','unit_price','ad'])
    for city in citys:
        print(f'processing data for city {city}')
        df = pd.read_csv(f'data/citys/{city}/renting.csv')
        df.insert(4,'city',city_name[city])
        df['price'],df['area'],df['orien'],df['room'],df['unit_price'],df['ad'] = zip(*df.apply(lambda x:ext_function(x),axis=1))
        df_new = pd.concat([df_new,df],join='inner', ignore_index=True)
    
    df_new = df_new.sort_values(by=['city','cbd','ad','brand'],ignore_index=True)
    df_new['orien'] = df_new['orien'].apply(lambda x:str(x).split('/'))

    # drop invalid
    df_new.drop(df_new[df_new['room'] <= 0].index,inplace=True)
    df_new.drop(df_new[(df_new['area'] <= 0) | (df_new['area'] > 1000)].index, inplace=True)
    df_new.drop(df_new[df_new['unit_price'] <= 1].index,inplace=True)

    df_new.to_csv(f'data/renting.csv')
...
def ext_function(row):
    ...
    return price,add['area'],add['orien'],add['room'],unit_price,is_ad(row['id'],row['ad_code'])
